...
c      if(itime.eq.3)stop
      ...
      if (ivar_dt.eq.1) then    ! nilai ivar_dt = 1 jadi ini digunakan
...
c
      ...
      do i=nbp1,np
		...
		! karena nilai iBC  = 2 maka loop ini Masuk 
...
c
      ...
      if (ivar_dt.eq.1) then ! ivar_dt nilainya sama dengan 1 jadi logika if true
	    call variable_time_step 
	    ddt_c=dt_new
